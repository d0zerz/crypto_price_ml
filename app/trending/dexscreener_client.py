        ...
        while retries < max_retries:
            try:
                ...
                if (
                    ...
                    wait_time = 60
                    logger.info(
                        f"HTTP 429 received. Retrying in {wait_time} seconds..."
                    )
        ...
        for pair in pairs:
            ...
            for timeframe in ["m5", "h1", "h6", "h24"]:
                if timeframe in pair.get("txns", {}):
                    aggregated_data[f"buys_{timeframe}"] += pair["txns"][timeframe].get(
                        "buys", 0
                    )
                    aggregated_data[f"sells_{timeframe}"] += pair["txns"][
                        timeframe
                    ].get("sells", 0)
            aggregated_data["volume_h24"] += pair.get("volume", {}).get("h24", 0)
            aggregated_data["price_change_h24"] += pair.get("priceChange", {}).get(
                "h24", 0
            )
