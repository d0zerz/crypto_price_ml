...
class JupiterQuoteScraper:
    ...
    async def _sample_exchange_rates(self, tokens: List[str]):
        ...
        logger.info(
            f"Starting exchange rate sampling at {start_time} for tokens {tokens}, thread {threading.get_ident()} ending at {end_time}"
        )
        ...
        while next_sample_time <= end_time:
            ...
            if current_time >= next_sample_time:
                ...
                for token in tokens:
                    try:
                        quote = await self.jup_client.get_buy_shitcoin_quote(
                            token, self.buy_amount
                        )
            ...
            if self.shutdown_flag.is_set():
                logger.info(
                    f"Shutting down scraping thread {threading.get_ident()} at {column_name} {current_time}"
                )
                break
