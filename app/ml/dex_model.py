...
class DexModel:
    ...
    def _generate_features(self, df_in: pd.DataFrame):
        df = df_in.copy()
        df.loc[:, 'buysell_m5_ratio'] = df['buys_m5'] / df['sells_m5'].replace(0, float('nan'))
        df.loc[:, 'buysell_h1_ratio'] = df['buys_h1'] / df['sells_h1'].replace(0, float('nan'))
        df.loc[:, 'market_cap_liquidity_ratio'] = df['market_cap'] / df['liquidity_usd'].replace(0, float('nan'))
        df.loc[:, 'market_cap_volume_ratio'] = df['market_cap'] / df['volume_h24'].replace(0, float('nan'))
    ...
    def _filter_data(self, in_df: pd.DataFrame):
        df = in_df.copy()
        df.drop(columns=['dex_id', 'T24h_diff_pct' ,'T8hr_diff_pct','T3hr_diff_pct','T1hr_diff_pct'], inplace=True)
    ...
    def train_crypto_models(self, df: pd.DataFrame ):
        ...
        randomForest = RandomForestClassifier(class_weight='balanced', n_estimators=2000, random_state=42)
        xgbClassifier = xgb.XGBClassifier(
            scale_pos_weight=(len(y_train) - sum(y_train)) / sum(y_train),  # Handling class imbalance (similar to class_weight='balanced')
        ...
        logisticRegression = LogisticRegression(class_weight='balanced', random_state=42, max_iter=1000)  # class_weight='balanced' helps with imbalance
        supportVectorMachine = SVC(class_weight='balanced', kernel='rbf', random_state=42)
        randomForestBalanced = BalancedRandomForestClassifier(n_estimators=200, random_state=42)
        eec = EasyEnsembleClassifier(n_estimators=50, random_state=42)

        models = [randomForest, randomForestBalanced, xgbClassifier, logisticRegression, supportVectorMachine, eec]
