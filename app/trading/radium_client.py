...
def api_swap():
    ...
    input_token_acc = next(
        (a for a in token_accounts if str(a["mint"]) == input_mint), None
    )
    output_token_acc = next(
        (a for a in token_accounts if str(a["mint"]) == output_mint), None
    )
    ...
    fee_response = requests.get(
        f"{API_URLS['BASE_HOST']}{API_URLS['PRIORITY_FEE']}"
    ).json()
    ...
    swap_transactions = requests.post(
        ...
        json={
            ...
            "outputAccount": (
                output_token_acc["publicKey"] if output_token_acc else None
            ),
        },
    ).json()
    ...
    all_tx_buf = [
        base64.b64decode(tx["transaction"]) for tx in swap_transactions["data"]
    ]
