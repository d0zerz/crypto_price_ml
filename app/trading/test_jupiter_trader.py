...
class TestJupiterTrader(unittest.TestCase):
    def setUp(self):
        # Initialize DexTrader with mock JupiterClient
    ...
    def test_should_sell_with_sample_data(self):
        ...
        # Test 1: Check if it detects the 20% drop from ATH
        # ATH is 1633.41, current is 2048.16
        # Drop = ((2048.16 - 1633.41) / 1633.41) * 100 = 25.4%
        self.assertTrue(self.trader.should_sell(test_prices),
                       "Should sell due to >20% drop from ATH")

        self.assertFalse(self.trader.should_sell(test_prices[:-1]),
                       "Should sell due to >20% drop from ATH")        
        
        # Test 2: Check with partial data to test plateau detection
        partial_prices = test_prices[:5]  # First 5 prices
        self.assertFalse(self.trader.should_sell(partial_prices),
        ...
        self.assertTrue(self.trader.should_sell(increasing_prices),
        ...
        self.assertFalse(self.trader.should_sell(stable_prices),
        ...
        self.assertTrue(self.trader.should_sell(plateau_prices),
    ...
    def test_edge_cases(self):
        ...
        self.assertFalse(self.trader.should_sell([]),
                        "Should not sell with empty price list")
        
        # Test with single price
        self.assertFalse(self.trader.should_sell([1000]),
                        "Should not sell with single price")
        
        self.assertTrue(self.trader.should_sell([1000, 1260]),
                       "Should sell with 25% increase from ATH")
        
        # Case 2: 19% increase (should not sell)
        self.assertFalse(self.trader.should_sell([1000, 1190]),
                        "Should not sell with <20% increase from ATH")
        
        # Case 3: 100% increase (should sell)
        self.assertTrue(self.trader.should_sell([1000, 500]),
                       "Should sell with 100% increase from ATH")
